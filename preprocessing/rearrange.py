...
def generate_dataset_file(dataset_name, dataset_root_path, output_file_path, compression_level='c23', perturbation = 'end_to_end'):
    ...
    if dataset_name == 'FaceForensics++' or dataset_name == 'DeepFakeDetection' or dataset_name == 'FaceShifter': 
        ...
        if os.path.isdir(actors_root):
            for compression_level in os.scandir(actors_root):
        ...
        if dataset_name == 'FaceForensics++':
            # Delete optional datasets if they exist
            for k in ['DFD_fake', 'DFD_real', 'FF-FH']:
                if k in dataset_dict['FaceForensics++']:
                    del dataset_dict['FaceForensics++'][k]
