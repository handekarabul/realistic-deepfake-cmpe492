...
def load_celebdf_test_videos(dataset_path, logger):
    """Load mp4 paths listed in Celeb-DF-v1/v2 List_of_testing_videos.txt."""
    list_file = Path(dataset_path) / 'List_of_testing_videos.txt'
    if not list_file.is_file():
        logger.error(f"Test list not found: {list_file}")
        sys.exit()
    movies_path_list = []
    missing = 0
    with open(list_file, 'r') as f:
        for line in f:
            ...
            if len(parts) < 2:
                ...
                continue
            rel_video = parts[1]
            video_path = Path(dataset_path) / rel_video
            if video_path.is_file():
                movies_path_list.append(video_path)
            else:
                missing += 1
                logger.warning(f"Test-list video not found: {video_path}")
    logger.info(
        f"Celeb-DF test-list: {len(movies_path_list)} videos found, {missing} missing"
    )
    return sorted(movies_path_list)


def _submit_preprocess_jobs(movies_path_list, mask_path, mode, num_frames, stride, logger):
    """Run face-crop jobs. dataset_path for each video is movie_path.parent (subfolder)."""
    if len(movies_path_list) == 0:
        logger.error("No videos to preprocess")
        sys.exit()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_processes) as executor:
        ...
        for movie_path in movies_path_list:
            per_video_mask = None
            dataset_path = movie_path.parent
            if mask_path is not None:
                if movie_path.stem not in [path.stem for path in masks_path_list]:
                    logger.error(f"No mask for video {movie_path}")
                per_video_mask = next((path for path in masks_path_list if path.stem == movie_path.stem), None)
                if per_video_mask is None:
...
def preprocess(dataset_path, mask_path, mode, num_frames, stride, logger):
    ...
    _submit_preprocess_jobs(movies_path_list, mask_path, mode, num_frames, stride, logger)
...
if __name__ == '__main__':
    ...
    test_list_only = config['preprocess'].get('test_list_only', {}).get('default', False)
   ...
   ## DeeperForensics-1.0
    ...
    if dataset_name in ['Celeb-DF-v1', 'Celeb-DF-v2'] and test_list_only:
        logger.info(f"Test-list-only preprocessing for {dataset_name}")
        movies_path_list = load_celebdf_test_videos(dataset_path, logger)
        _submit_preprocess_jobs(movies_path_list, None, mode, num_frames, stride, logger)
    elif 'sub_dataset_paths' in globals() and len(sub_dataset_paths) != 0:
