...
class Trainer(object):
    ...
    def train_epoch(
        ...
        for iteration, data_dict in tqdm(enumerate(train_data_loader), total=len(train_data_loader)):
            ...
            if iteration % 300 == 0 and self.config['local_rank'] == 0:
                if self.config.get('SWA', False) and self.scheduler is not None:
                    if epoch > self.config['swa_start'] or self.config['dry_run']:
                        self.scheduler.step()
                ...
                for _, recorder in train_recorder_loss.items():
                    recorder.clear()
                for _, recorder in train_recorder_metric.items():
                    recorder.clear()
    ...
    def save_best(self, epoch, iteration, step, losses_one_dataset_recorder, key, metric_one_dataset):
        ...
        if 'pred' in metric_one_dataset:
            ...
            if 'balanced_acc_opt' in metric_one_dataset:
                metric_str += (
                    f'; opt_thr:{metric_one_dataset["opt_threshold"]:.4f}'
                    f'; balanced_acc_opt:{metric_one_dataset["balanced_acc_opt"]:.4f}'
                    f'; acc_real_opt:{metric_one_dataset["acc_real_opt"]:.4f}'
                    f'; acc_fake_opt:{metric_one_dataset["acc_fake_opt"]:.4f}'
                )
                writer.add_scalar(f'test_metrics/balanced_acc_opt', metric_one_dataset['balanced_acc_opt'], global_step=step)
                writer.add_scalar(f'test_metrics/acc_real_opt', metric_one_dataset['acc_real_opt'], global_step=step)
                writer.add_scalar(f'test_metrics/acc_fake_opt', metric_one_dataset['acc_fake_opt'], global_step=step)
